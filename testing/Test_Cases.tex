...
\subsection*{Description}
Given a set of web pages containing duplicate links and multiple paths leading to the same URL, verify that the crawler does not crawl (fetch/process) the same URL more than once during a single execution.
...
\begin{itemize}
  ...
  \item A controlled test website (or local test server) exists with the following pages:
    \begin{itemize}
      ...
      \item \texttt{/a.html} contains a link to \texttt{/c.html}.
      \item \texttt{/b.html} also contains a link to \texttt{/c.html} (duplicate target via another path).
...
\begin{enumerate}
  ...
  \item Count the number of times \texttt{/c.html} appears in the visited/fetched URL list.
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
  \item No errors occur.
  \item Each unique URL is fetched/processed at most once during the execution.
  \item The URL \texttt{/c.html} appears exactly once in the visited/fetched URL list, even though it is linked multiple times and via multiple paths.
\end{itemize}
