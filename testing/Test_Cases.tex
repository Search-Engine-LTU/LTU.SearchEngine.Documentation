...
\section{Test Case: TC-FRQ-2007}

\subsection*{Related Requirements}
FRQ-2007

\subsection*{Description}
Verify, at a unit level, that the indexing component normalizes textual terms into a global, language-neutral format before they are committed to the inverted index. This ensures that search queries are case-insensitive and insensitive to morphological variations (e.g., stemming).

\subsection*{Preconditions}
\begin{itemize}
  \item The test isolates the \textbf{IndexerService} (System Under Test).
  \item External dependencies (\textit{ITermRepository}, \textit{ILanguageAnalyzer}) are replaced with \textbf{Mock Objects}.
  \item The system is configured with a standard normalization pipeline (Lowercasing, Stemming, Stop-word removal).
\end{itemize}

\subsection*{Test Steps}
\begin{enumerate}
  \item \textbf{Arrange (Normalization):} Prepare a set of input strings with mixed casing and different grammatical forms (e.g., "Running", "RUN", "runs").
  \item \textbf{Act:} Call the index method on the \textit{IndexerService} with the prepared strings and a target Page ID.
  \item \textbf{Assert (Case Insensitivity):} Verify that "Running" and "run" result in the exact same term being sent to \textit{MockTermRepository}.
  \item \textbf{Assert (Stemming):} Verify that morphological variations (e.g., "crawled", "crawling") are reduced to their base form (e.g., "crawl") before storage.
  \item \textbf{Arrange (Noise Reduction):} Prepare a string containing common stop-words (e.g., "the", "is", "and").
  \item \textbf{Act:} Call the index method with the noise-heavy string.
  \item \textbf{Assert (Filtering):} Verify that the \textit{MockTermRepository.StoreTerm()} method is not called for identified stop-words, ensuring only significant terms are indexed.
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
  \item All indexed terms are stored in lowercase, stripped of language-specific suffixes (stemmed), and filtered for noise.
  \item Redundant grammatical variations of the same word map to a single, normalized entry in the global index.
\end{itemize}
% ==========================================================
...
\section{Test Case: TC-NFRQ-6006}

\subsection*{Related Requirements}
FRQ-6006, FRQ-1004

\subsection*{Description}
Verify that rate limiting parameters (such as minimum delay between requests and maximum concurrency) 
can be configured via a configuration file and that these settings are correctly applied by 
the system after a restart.

\subsection*{Preconditions}
\begin{itemize}
    \item Crawler componen is operational.
    \item The syetem uses a configuration file (e.g., appsettings.json) to store MinDelayMs and 
    MaxConcurrencyPerDomain.
    \item A test webserver (or mock) is available to log the arriving time of incoming HTTP request.
    \item Access to system logs is enabled to verify internal state of the crawler.
\end{itemize}

\subsection*{Test Steps}
\begin{enumerate}
    \item \textbf{Initial Configuration:} Set minDelayMs to 1000 (1 second) in the Configuration file.
    \item \textbf{System Restart:} Restart the application to ensure that the configuration is loaded.
    \item \textbf{Execute Crawl:} Issue a crawl command for a seed URL that contains at least three internal links on the same domain.
    \item \textbf{Measure Timing:} Record the timestamps of the HTTP requests sebt by the crawler.
    \item \textbf{Re-Configure:} Change MinDelayMs to 100 (0.1 seconds) in the configuration file.
    \item \textbf{System Restart:} Restart the application.
    \item \textbf{Execute Second Crawl:} Repeat the crawl process and record the timestamps.
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
    \item \textbf{Initial Run:} The time difference between consecutive requests to the same domain is consistently >= 1000 ms. 
    \item \textbf{Second Run:} The time difference between consecutive requests to the same domain is consistently >= 100 ms. 
    \item \textbf{Verification:} The system correctly reads the values from the configuration file without requiring code changes or recompilation. 
\end{itemize}
