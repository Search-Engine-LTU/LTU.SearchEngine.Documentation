...
(e.g., URLs ending with \texttt{.pdf} or served with \texttt{Content-Type: application/pdf})
...
\begin{itemize}
  ...
  \item The crawler has logging/telemetry enabled OR exposes a list/queue of discovered URLs.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Observe the crawler's discovered links output (e.g., logs, discovered-URL list, or crawl queue).
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
  \item The crawler identifies the hyperlink target as a PDF resource.
  \item The discovered URL \texttt{https://example.test/docs/report.pdf} is recorded as a PDF link
        (e.g., tagged as \texttt{PDF}, added to a PDF-discovery list, or enqueued to a PDF fetch queue).
  \item No indexing step is required for this test case; the pass criterion is detection/registration only.
  \item No errors occur during HTML parsing and link classification.
\end{itemize}
% ==========================================================
...
\begin{enumerate}
    ...
    \item \textbf{Assert (Sequence):} Verify that \textit{MockRepository.CreateRecord()} was called exactly once.
    ...
    \item \textbf{Assert (Resilience):} Verify that \textit{MockRepository.CreateRecord()} is still called exactly once, ensuring the logic proceeds despite the fetch exception.
\end{enumerate}
