...
\subsection*{Description}
Verify that PDF documents discovered by the crawler are processed by the indexing
component and that their textual contents are extracted and added to the search index,
making the PDF searchable by its contained terms.
...
\begin{itemize}
  ...
  \item The seed HTML page and the linked PDF file are allowed by robots.txt and any configured domain allowlist.
  \item The PDF file is a text-based PDF (not scanned, no OCR required).
  \item The search index is cleared before the test or a dedicated temporary test index is used.
\end{itemize}

\subsection*{Test Data}
\begin{itemize}
  \item Seed URL: An HTML page containing a hyperlink to \texttt{sample.pdf}.
  \item PDF file contains a known unique term: \texttt{``InformationRetrievalTestTerm''}.
\end{itemize}
...
\begin{enumerate}
  \item Configure the crawler with the HTML page as the seed URL.
  \item Start the crawling process.
  \item Allow the crawler to fetch the HTML page and discover the link to the PDF file.
  \item Allow the indexing component to download the PDF and extract its textual contents.
  \item Wait until the crawl queue is empty and the indexing process reports completion.
  \item Perform a search query for the known term contained in the PDF.
\end{enumerate}

\subsection*{Expected Results}
\begin{itemize}
  \item The crawler successfully discovers the PDF link.
  \item The indexing component downloads the PDF and extracts its textual content.
  \item Extracted terms are stored in the inverted index.
  \item A search for \texttt{``InformationRetrievalTestTerm''} returns the PDF document (identified by its URL or document ID) exactly once.
  \item No parsing, crawling, or indexing errors related to the PDF occur in the system logs.
\end{itemize}
% ==========================================================
...
\subsection*{Test Steps}
\begin{enumerate}
    \item Enter the phrase query \texttt{"hello dolly"} into the search input.
    \item Execute the search.
...
\subsection*{Expected Results}
\begin{itemize}
    \item Only Document A is returned.
    \item Documents where the words appear separately or in a different order are not returned.
    \item The system treats the quoted phrase as a single search unit.
