...
FRQ-3002 & A query must be able to match more then one set of characters, if the option (AND) is chosen. \newline
            Example: If the query searches for "cat AND dog", results must include items containing both 
            "cat" and "dog", and exclude items containing only one of them. & \\
FRQ-3003 & The query shall support optional characters using the OR operator. \newline
            ...
            include items containing either "cat", "dog", or both.                         & \\
...
FRQ-3008 & Search results shall be ranked by relevance using a combination of TF/IDF scores and PageRank. \newline
- Documents with higher TF/IDF scores for the query terms must appear before documents with lower scores. \newline
- When TF/IDF scores are equal, documents with higher PageRank shall appear first. \newline
...
NFRQ-6004 & The system shall provide a search API for the UI. & \\
\bottomrule
\end{longtable}
