...
The crawler shall restrict all crawling to domains explicitly listed in a configurable domain whitelist.\newline
  The crawler shall enforce a configurable maximum number of concurrent HTTP requests per domain (maxConcurrencyPerDomain) 
  and a configurable minimum delay (minDelayMs) between consecutive requests to the same domain.\newline
  The system shall identify crawling targets by their domain name (e.g., ltu.se) rather than resolved IP addresses 
  to ensure consistent behavior if underlying hosting or IP mappings change. & TC-FRQ-1003-A \newline TC-FRQ-1003-B\\

FRQ-1004 & The crawler shall log the active rate limiting configuration values (\texttt{maxConcurrencyPerDomain}, \texttt{minDelayMs}) at startup for verification and debugging purposes. & TC-FRQ-1004 \\
FRQ-1005 & The crawler must parse and adhere to the ``robots.txt'' file of the target domain. & TC-FRQ-1005 \\


FRQ-1006 & The crawler shall avoid crawling the same URL more than once per execution. & TC-FRQ-1006 \\

FRQ-1007 & The crawler shall ignore non-relevant resources (images, CSS, JS). & TC-FRQ-1007\\

FRQ-1008 & The system must support adding new domains to the whitelist via a configuration file. & TC-FRQ-1008\\

FRQ-1009 & The crawler must detect linked PDF files and include them in the index. & TC-FRQ-1009-A \newline TC-FRQ-1009-B \\
...
FRQ-2005 & The system shall enqueue a URL in the crawl queue before any fetch attempt is performed. & TC-FRQ-2005 \\
FRQ-2006 & The system shall create a page record only after a fetch attempt has been completed, regardless of fetch success. & TC-FRQ-2006 \\
FRQ-2007 & The system shall store indexed terms in a global, language-normalized form. & TC-FRQ-2007 \\
...
FRQ-3002 & Single Term Support. The system shall support single-term queries, where a term consists of one word (e.g., test, hello).& TC-FRQ-3002\\
FRQ-3003 & Phrase Support. The system shall support phrase queries, defined as multiple words enclosed in double quotation marks (e.g., "hello dolly").& TC-FRQ-3003\\

FRQ-3004 & Operator Case Sensitivity. The system shall require Boolean operators to be specified in uppercase letters. & TC-FRQ-3004 \\
...
2. Symbol: \texttt{||} \newline
3. Implicit: Whitespace between terms implies OR (Default behavior).& TC-FRQ-3005 \\
...
FRQ-3007 & Required Term Operator. The system shall support the required (+) operator, indicating that the term must exist in a matching document. \newline
            (e.g, If the query searches for ”+"are cats" dog”, results must include items containing ”are cats” and can but does not have to contain ”dog”). & TC-FRQ-3007\\
...
FRQ-3009 & Grouping with Parentheses. The system shall support grouping of query expressions using parentheses to control operator precedence. \newline
            (e.g, If the query searches for ("cat" AND "dog") OR "fish", results should include items containing either both ”cat” and "dog", "fish" or both clauses.). & TC-FRQ-3009\\
...
FRQ-3010 & Special Character Escaping. The system shall support escaping of special query characters using a backslash (\textbackslash). \newline
            Supported Escapable Characters: \newline \texttt{+ - \&\& || ! ( ) \{ \} [ ] \^{} " \textasciitilde{} * ? : \textbackslash} & TC-FRQ-3010 \\
